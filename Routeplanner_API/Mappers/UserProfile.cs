        ...
        public UserProfile()
        {
            ...
            CreateMap<User, UserDto>()
                ...
                .ForMember(dest => dest.PasswordHash, opt => opt.MapFrom(src => 
                    src.UserConfidential != null ? src.UserConfidential.PasswordHash : null));
            ...
            CreateMap<UserDto, User>()
                ...
                .AfterMap((src, dest) => {
                    ...
                    dest.UserConfidential.PasswordHash = src.PasswordHash;
                })
            ...
            CreateMap<CreateUserDto, User>()
                // Assuming UserId can be client-provided for new users.
                // If UserId is database-generated, this .ForMember should be .Ignore().
                .ForMember(dest => dest.UserId, opt => opt.MapFrom(src => src.UserId))
                .ForMember(dest => dest.Username, opt => opt.MapFrom(src => src.Username))
                .ForMember(dest => dest.CreatedAt, opt => opt.MapFrom(_ => DateTime.UtcNow))
                .ForMember(dest => dest.UserConfidential, opt => opt.MapFrom(src => new UserConfidential
                {
                    // UserId for UserConfidential is taken from CreateUserDto, same as User.UserId.
                    UserId = src.UserId, 
                    Email = src.Email,
            ...
            CreateMap<UpdateUserDto, User>()
                ...
                .ForMember(dest => dest.UserId, opt => opt.Ignore()) // PK should not change.
                .ForMember(dest => dest.CreatedAt, opt => opt.Ignore()) // Creation timestamp should not change.
                .ForMember(dest => dest.Locations, opt => opt.Ignore())
                .ForMember(dest => dest.Routes, opt => opt.Ignore())
                .ForMember(dest => dest.Right, opt => opt.Ignore())
            ...
            // AfterMap to handle UserConfidential updates conditionally for UpdateUserDto.
            // This ensures that Email and PasswordHash are only updated if provided in the DTO.
            CreateMap<UpdateUserDto, User>() // This is a continuation of the above map, effectively. AutoMapper merges configurations for the same type pair.
                .AfterMap((src, dest) => {
                    // Only modify UserConfidential if Email or PasswordHash is provided in UpdateUserDto.
                    if (src.Email != null || src.PasswordHash != null)
                    {
                        if (dest.UserConfidential == null)
                        {
                            // Assumes dest.UserId is populated on the User entity being mapped to.
                            dest.UserConfidential = new UserConfidential { UserId = dest.UserId };
                        }
                        if (src.Email != null)
                        {
